...
class KnowledgeBaseManager:
    ...
    def create_knowledge_bases(self):
        ...
        if not self._knowledge_bases_path.exists():
            logging.warning(f"Knowledge bases path {self._knowledge_bases_path} does not exist")
            return
    ...
    def _insert_documents_from_directory(self, directory: Path, vector_db_id: str):
        ...
        if rag_documents:
            try:
                logging.info(f"Inserting {len(rag_documents)} documents into {vector_db_id}")
        ...
        try:
            ...
            for vector_db in vector_dbs:
                try:
                    ...
                    logging.error(f"Failed to delete vector database {vector_db.vector_db_id}: {str(e)}")
