"""Zammad trigger webhook: verify signature, dedupe, forward to Request Manager."""
...
def verify_zammad_webhook_signature(
    ...
    if not secret.strip():
        logger.error(
            "ZAMMAD_WEBHOOK_SECRET is empty — webhook verification cannot succeed "
            "(set ticketingZammad.webhookSecret or ZAMMAD_WEBHOOK_SECRET)"
        )
        return False
...
class ZammadService:
    ...
    def __init__(self) -> None:
        ...
        if not self.webhook_secret:
            logger.warning(
                "ZAMMAD_WEBHOOK_SECRET is not set: POST /zammad/webhook will return 401 until "
                "the secret is configured (ticketingZammad.webhookSecret or env)."
            )
    ...
    async def handle_webhook(
        ...
        *,
        ...
        ai_agent_uid = (os.getenv("ZAMMAD_AI_AGENT_USER_ID") or "").strip()
        if ai_agent_uid and str(created_by_id) == ai_agent_uid:
            logger.info(
                "Zammad webhook: skipping article from configured AI agent user (feedback loop)",
                ticket_id=ticket_id,
