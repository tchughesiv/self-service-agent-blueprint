...
class WebhookIntegrationHandler(BaseIntegrationHandler):
    ...
    async def deliver(
        ...
        try:
            ...
            # Use default webhook URL if no user-specific URL is configured
            if not url:
                import os

                url = os.getenv("DEFAULT_WEBHOOK_URL")

            if not url:
                return IntegrationResult(
                    success=False,
                    status=DeliveryStatus.FAILED,
                    message="No webhook URL configured (neither user-specific nor default)",
                )
    ...
    async def is_configured(self) -> bool:
        """Check if webhook integration has default configuration."""
        import os

        # Check for default webhook URL - if present, integration is configured
        default_webhook_url = os.getenv("DEFAULT_WEBHOOK_URL")
        if default_webhook_url:
            return True

        # If no default URL, users would need to configure it individually
        return False

    async def health_check(self) -> bool:
        """Webhook integration health check - only available when configured."""
        # Only return True if the integration is properly configured
