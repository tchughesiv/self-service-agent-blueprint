    ...
    metrics = [
        ...
        ConversationalGEval(
            ...
            evaluation_steps=[
                ...
                "IMPORTANT: This is a multi-agent system where users can be routed between a routing agent and specialist agents.",
                "IMPORTANT: For Zammad ticket flows (ticket-review / ticket-laptop-refresh / ticket-general-support), the agent should NOT offer 'return to routing agent' or similar — directing the user to open a new ticket for a different issue is acceptable and CORRECT.",
                ...
                "  - If user asks an out-of-scope question (e.g., 'what is the fastest bird'), acceptable responses include:",
                "    1. (Non-ticket / DM flows) Offer to return to router for help with other topics — CORRECT where routing applies",
                "    2. Politely redirect user back to the original topic/question — CORRECT",
                "    3. (Zammad ticket threads) Explain this ticket is for the stated topic and suggest opening a new ticket for unrelated issues — CORRECT",
                "  - Do not penalize ticket-channel agents for omitting return-to-router language.",
                "CRITICAL: When evaluating explicit return-to-router requests:",
                "  - For standalone laptop-refresh / routing-agent flows: emitting 'task_complete_return_to_router' when appropriate is CORRECT.",
                "  - For Zammad ticket threads: staying on the ticket specialist and explaining that a different issue requires a new ticket is CORRECT (do not expect task_complete_return_to_router).",
                "  - After returning to router (where applicable), the routing agent re-greeting is EXPECTED and CORRECT",
                "Assess if all relevant user requests were addressed:",
                "  - Laptop refresh requests should result in eligibility check, options, selection, and ticket creation where applicable",
                "  - Out-of-scope handling should match channel (ticket vs non-ticket) per steps above",
                "The conversation is complete if all user intentions were properly handled, including routing operations where that channel supports routing.",
            ],
        ),
        ConversationalGEval(
