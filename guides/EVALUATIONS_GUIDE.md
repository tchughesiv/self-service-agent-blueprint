...
1. Script calls GET /api/v1/conversations (optional filters: limit, user_email, session_id, etc.)
...
```bash
cd evaluations
# Export up to 20 sessions (default); uses REQUEST_MANAGER_URL or http://localhost:8080
uv run export_conversations_from_api.py

# With filters
uv run export_conversations_from_api.py --limit 10 --user-email john.doe@company.com
