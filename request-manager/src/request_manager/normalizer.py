...
class RequestNormalizer:
    ...
    def _normalize_slack_request(
        ...
        return NormalizedRequest(
            **base_data,
            ...
            requires_routing=self._requires_agent_routing(request.content),
        )
    ...
    def _normalize_base_request(
        ...
        integration_context = {
            "platform": request.integration_type.value,
    ...
    def _requires_agent_routing(self, content: str) -> bool:
        """Determine if the request requires agent routing."""
        # Simple heuristics to determine if routing is needed
        # This could be enhanced with NLP or more sophisticated logic
        
        routing_keywords = [
            "help", "what can you do", "options", "menu", "start", "hello", "hi"
        ]
        
        content_lower = content.lower()
        
        # If content contains routing keywords, it likely needs routing
        if any(keyword in content_lower for keyword in routing_keywords):
            return True
        
        # Short messages often need routing
        if len(content.split()) < 3:
            return True
        
        # Default to requiring routing for safety
        return True
    ...
    def _parse_user_agent(self, user_agent: str) -> Dict[str, Any]:
        ...
        context["is_mobile"] = any(indicator in user_agent_lower for indicator in mobile_indicators)
