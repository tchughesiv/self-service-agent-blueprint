...
async def require_admin_access(
    ...
    """Require admin access for conversation review endpoints.

    Checks for admin groups or audit API keys.
    Users cannot access their own conversations - only admins can access any conversation.
    """
    if not current_user:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Authentication required"
    ...
    admin_groups = ["admin", "audit", "support", "review"]
    is_admin = any(group in admin_groups for group in groups)

    # Check for audit API keys
    if not is_admin:
        audit_api_keys_env = os.getenv("AUDIT_API_KEYS", "")
        if audit_api_keys_env:
            audit_api_keys = [
                key.strip() for key in audit_api_keys_env.split(",") if key.strip()
            ]
            api_key = current_user.get("token", "")
            is_admin = api_key in audit_api_keys

    if not is_admin:
        logger.warning(
            "Non-admin attempt to access conversation review",
...
async def get_conversations(
    ...
    """Get conversations with flexible filtering. Admin only.

    This endpoint allows admins to review conversations for quality assurance,
    debugging, and audit purposes. Users cannot access their own conversations.
    ...
    for session in sessions:
        ...
        if include_messages:
            ...
            if session.current_agent_id != agent_id:
                continue
