...
class SessionManager:
    ...
    async def get_user_sessions(
        ...
        if integration_type:
            stmt = stmt.where(RequestSession.integration_type == integration_type)
        if status:
            stmt = stmt.where(RequestSession.status == status)
            
        stmt = stmt.order_by(RequestSession.last_activity_at.desc()).limit(limit)
    ...
    async def cleanup_inactive_sessions(self, hours: int = 24) -> int:
        ...
        stmt = (
            update(RequestSession)
            .where(
                RequestSession.last_activity_at < cutoff_time,
                RequestSession.status == SessionStatus.ACTIVE,
            )
            .values(
    ...
    async def _find_active_session(
        ...
        stmt = stmt.order_by(RequestSession.last_activity_at.desc())
